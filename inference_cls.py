...
def image_inferences(img_path , engine , iou_threshold , conf_threshold , label , color_palette , show):
    ...
    with engine.create_execution_context() as context:
        ...
        im = im.transpose((2, 0, 1))
        im = (2.0 / 255.0) * im - 1.0
        np.copyto(input[0]['host'],im.ravel())
        cuda.memcpy_htod_async(input[0]['device'],input[0]['host'].stream)
        context.execute_async(bindings=bindings,stream_handle=stream.handle)
        cuda.memcpy_dtod_async(input[0]['host'],input[0]['device'].stream)
        stream.synchronize()

        output_data=np.array(output[0]['host'])
        print(output_data)
        return output_data
